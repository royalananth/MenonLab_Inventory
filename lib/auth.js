...
export const caps = (m) => {
  if (!m) return { approve: false, chair: false, chairBackup: false, place: false, grants: false, edit: false, reports: false, access: false, owner: false, book: false, request: false, view: false, guest: true };
  ...
  return {
    ...
    // Stage one: the PD who owns the project approves and proposes the funding.
    approve: (chair || pd) && !guest,
    // Stage two: only Dr. Menon signs off above the threshold. The access owner
    // is a deliberate backup so nothing stalls while he is away — it is logged
    // as acting on his behalf, not as him.
    chair: chair && !guest,
    chairBackup: (chair || owner) && !guest,
    // Only Megan places the order on the UTMB site. The owner can step in.
    place: (purchasing || owner) && !guest,
    grants: (chair || pd) && !guest,           // set budgets
    edit: (full || chair) && !guest,           // edit inventory, instruments
    reports: (chair || pd || purchasing || full) && !guest,
