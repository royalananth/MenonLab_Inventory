...
async function doInit() {
  ...
  // Dr. Menon, 10 Sep 2026: requests go to Megan, carrying the name of the PI
  // she should route them to for FRS / grant assignment.
  await q(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS requested_approver text`);
  ...
  if (seed5.rows.length === 0) {
    ...
    // Purchasing desk: orders land with Megan, who routes them on.
    await q(`INSERT INTO members (name,email,role,pd) VALUES ('Megan','','purchasing',false)
             ON CONFLICT (name) DO UPDATE SET role='purchasing'`);
